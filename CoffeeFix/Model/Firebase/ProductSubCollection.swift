...
class ProductSubCollection{
    
    private static let db = Firestore.firestore() /// Initiates the connection to firestore
    private static let parentCollectionName = "coffeeshops" /// We save our collection name in a variable
    ...
    static func startListener(id: String, completion: @escaping ()->()){
        ...
        listener = db.collection("\(parentCollectionName)/\(id)/\(collectionName)").addSnapshotListener { (snap, error) in
            if let snap = snap {
